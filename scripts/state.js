...
// ==== AUTO FIX: robust product helpers ====
const get = (p, ...keys) => {
...
const isActive = (p) =>
  String(get(p,"IsActive","isActive","active")||"").toLowerCase()==="true";

const getState = (p) =>
  String(get(p,"State","state")||"").trim();

const getChildrenStatus = (p) =>
  String(get(p,"ChildrenStatus","childrenStatus")||"").trim();

const getPrice = (p) => {
  const n = Number(get(p,"PriceUSD","priceUSD","price"));
...
const STATE_NAME = STATE_SLUG.replace(/-/g," ").replace(/\b\w/g,c=>c.toUpperCase());

const STATE_KEY = STATE_SLUG.replace(/-/g," ").trim().toLowerCase();
(() => {
  ...
  const stateList = list.filter(p =>
    String(p?.State || p?.Subcategory || "").trim().toLowerCase() === STATE_KEY
  );
